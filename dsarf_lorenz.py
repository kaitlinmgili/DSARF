...
from plot_results_lorenz import plot_result
...
class DMFA(nn.Module):
    ...
    def __init__(self, n_data=100, T = 18,  factor_dim=10, z_dim=5,
        ...
        self.q_s = nn.Parameter(self.softmax(torch.rand(n_data, T, S)),requires_grad=False)
        ...
        #self.q_F_loc_mu = (torch.rand(factor_dim, D)- 1/2)
...
#            = dmfa.forward(mini_batch, u_vals, mini_batch_idxs)
        ...
        fig = plt.figure()
        idx = dmfa.q_s[0].argmin(dim=-1).detach().numpy()==0
        xs = dmfa.q_z_mu.detach().numpy()[0,:,0]
        ys = dmfa.q_z_mu.detach().numpy()[0,:,1]
        zs = dmfa.q_z_mu.detach().numpy()[0,:,2]
        ax = fig.gca(projection='3d')
        ax.plot(xs[idx], ys[idx], zs[idx], lw=0.5)
        ax.plot(xs[~idx], ys[~idx], zs[~idx], lw=0.5)
        ax.set_xlabel("X Axis")
        ax.set_ylabel("Y Axis")
        ax.set_zlabel("Z Axis")
        ax.set_title("Lorenz Attractor")
        fig.savefig(fig_PATH+'lorenz.png', bbox_inches='tight')
        ...
        for j in range(n_vis, 0 , -1):
            ...
            for i in range(max(dmfa.L), T, 1):
                ...
                z_val_p = (p_s.reshape(-1, 1, 1) * p_z_mu+p_z_sig.exp()*3).sum(dim=0)
                z_val_n = (p_s.reshape(-1, 1, 1) * p_z_mu-p_z_sig.exp()*3).sum(dim=0)
        ...
        for i in range(factor_dim):
            ...
            #r'Projected true latent space: $Y\hat{F}^{+}$'
            ax.plot(xy[:,i], color='blue',label='True (projected latent space)',linewidth=1)
            ...
            if i==0:
                ax.legend(fontsize=12, loc='lower right',
                          framealpha=0, #borderaxespad=-1.5,
                          ncol=2, bbox_to_anchor=[1., 0.92], #loc=3,
                          #mode="expand",
                          borderaxespad=0.)
        ...
        if acc_states>50:
            q_s=1-q_s
        q_s = [np.median(q_s[0].astype('float')[i:min(i+5, 1000+1)]) for i in range(1000)]
        q_s=np.concatenate((np.zeros(5//2+1),q_s[:-(5//2)-1])).reshape(1,-1)
#        q_s = np.load(fig_PATH+'q_s.npy')
#        q_s[0,:3]=1
#        q_s_rslds = np.load(fig_PATH+ 'z_rslds_lorenz.npy').reshape(1,-1)
#        q_s_slds = np.load(fig_PATH+ 'z_slds_lorenz.npy').reshape(1,-1)
#        q_s_true = np.load(fig_PATH+ 'z_lorenz.npy').reshape(1,-1)
        q_s_all = q_s
        for i in range(1):
            ...
            ax.spines['top'].set_visible(False)
            ax.spines['right'].set_visible(False)
            ax.set_ylabel(methods[i], fontsize=fontsize-2)
            #ax.set_xticks([])
        ax.tick_params(axis='both', which='major', labelsize=fontsize)
        ...
        fontsize=21
        smp = 2
        fig = plt.figure(figsize=(7,7))#figsize=(7/2.5*5,7/2.5*2.5))
        ax = fig.gca(projection='3d')
        if acc_states>50:
#            idx = np.load(fig_PATH+'q_s.npy')[0]==1
#            idx[:3]=True
            idx = dmfa.q_s[0].argmin(dim=-1).detach().numpy()==1
        else:
#            idx = np.load(fig_PATH+'q_s.npy')[0]==0
            idx = dmfa.q_s[0].argmin(dim=-1).detach().numpy()==0
        idxs = [idx[1:], ~idx[1:]]
        xy = dmfa.q_z_mu.detach().numpy()[-n_vis:].reshape(-1,z_true.shape[-1])[:-1]
        dxydt_m = ws[1:]-xy
        for k in range(S):
            ax.quiver(xy[idxs[k], 0][::smp], xy[idxs[k], 1][::smp], xy[idxs[k], 2][::smp],
                          dxydt_m[idxs[k], 0][::smp], dxydt_m[idxs[k], 1][::smp],dxydt_m[idxs[k], 2][::smp],
                          color=colors[k % len(colors)], alpha=1.0, label='State %d'%k,
                          linewidths=1, length=2)#, normalize=True, 
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_zticks([])
        ax.set_xlabel(r"$w^0$",fontsize=fontsize+0.5)
        ax.set_ylabel(r"$w^1$",fontsize=fontsize+0.5)
        ax.set_zlabel(r"$w^2$",fontsize=fontsize+0.5)
        ax.xaxis.labelpad=-9
        ax.yaxis.labelpad=-9
        ax.zaxis.labelpad=-9
        ax.set_title('Inferred Dynamics', pad=-20)
        ax.grid(False)
        #ax.axis('off')
        ax.set_facecolor('w')
        ax.legend(fontsize=14, framealpha=0,loc='lower center',bbox_to_anchor=[.58, 0.1])
        ax.tick_params(axis='both', which='major', labelsize=fontsize-2)
        fig.savefig(fig_PATH+'_all_traj_state.pdf', bbox_inches='tight')
        plt.close('all')
        ...
        ax.plot(xy[:, 0][::smp], xy[:, 1][::smp], xy[:, 2][::smp],
        ...
        ax.yaxis.labelpad=-9
        ax.zaxis.labelpad=-9
        ax.set_title('True Latent Space', pad=-20)
        ax.grid(False)
        #ax.axis('off')
        ax.set_facecolor('w')
        ax.tick_params(axis='both', which='major', labelsize=fontsize-2)
        fig.savefig(fig_PATH+'_all_traj_true.pdf', bbox_inches='tight')
        plt.close('all')
