...
#            right=False, left=False, labelleft=False) # labels along the bottom edge are off
    ...
    if predict or days == 0:    
        ...
        for idx_loc in range(0, D, D//5):
            ...
            plt.tight_layout()
            fig.savefig(fig_PATH + "%sprediction_long_term_loc_%s" %(prefix,idx_loc) + ext, bbox_inches='tight')
        plt.close('all')

        for idx_t in range(0, T_b, T_b//5):
            fig = plt.figure(figsize=(10,11))
            ax = fig.add_subplot(3,1,1)
            ax.imshow(dataa[-1].reshape(-1, 30, 84)[idx_t], cmap = "jet")
            ax.set_ylabel("Actual", fontsize=fontsize+2)
            ax = fig.add_subplot(3,1,2)
            ax.imshow(y_pred[:T_b].reshape(-1,30,84)[idx_t], cmap='jet')
    ...
    if predict or days == N:
        ...
        for idx_loc in range(0, D, D//20):
            ...
            plt.tight_layout()
            fig.savefig(fig_PATH + "%s_all_prediction_roll_short%s" %(prefix,idx_loc) + ext, bbox_inches='tight')
            plt.close('all')

        for j in range(days, 0 , -1):
            for idx_t in range(0, T, T//5):
                fig = plt.figure(figsize=(10,7/2.5*1.5))
                ax = fig.add_subplot(2,1,1)
                ax.imshow(dataa[-j].reshape(-1, 30, 84)[idx_t], cmap = "jet")
                ...
                plt.tick_params(
                    ...
                    labelbottom=False,
                    right=False, left=False, labelleft=False) # labels along the bottom edge are off
                fig.savefig(fig_PATH + "_all_%s(-%d)_prediction_roll_short_time_%s" %(prefix,j,idx_t) + ext, bbox_inches='tight')
            plt.close('all')
        ...
        for j in range(days, 0 , -1):
            for idx_t in range(0, T, T//5):
                ...
                ax.set_ylabel("Actual", fontsize=fontsize+2)
                ax = fig.add_subplot(3,1,2)
                ax.imshow(y_recv[-j].reshape(-1,30,84)[idx_t], cmap='jet')
                ax.set_ylabel("Recovered", fontsize=fontsize+2)
                y_preds = y_pred[-j]*1.0
                ax = fig.add_subplot(3,1,3)
                ax.imshow(y_preds.reshape(-1,30,84)[idx_t], cmap='jet')
                ...
                ax.set_title('Pacific sea temperature'+' #%d'%idx_t, fontsize=fontsize)
                plt.tight_layout()
                fig.savefig(fig_PATH + "%s(-%d)prediction_roll_short_time_%s" %(prefix,j,idx_t) + ext, bbox_inches='tight')
            plt.close('all')
