...
def plot_result(dmfa, classes,
    ...
    if ID is not None:
        ID_name = np.array(['bat','guangzhou','birmingham','hangzhou','seattle', 'flu','prec', 'dengue'])
...
#            right=False, left=False, labelleft=False) # labels along the bottom edge are off
    ...
    if True:    
        if ID is not None:
            titles = ['Joint',
                      ...
                      'Seattle loop detector', 'Flu cases','precipitation', 'Dengue cases']
            ylabels = [['Location x', 'Location y', 'Location z'],
                       ['Traffic speed'],
                       ['Occupancy'],
                       ['Passenger flow'],
                       ['Traffic speed'],['New cases'],['mm'],['Relative activity']]    #Relative activity
        fontsize= 12
    ...
    if True:
        ...
        ticks = [np.arange(144/2, 144*5, 144),
                 np.arange(18/2, 126, 18),
                 np.arange(108/2, 540, 108),
                 np.arange(288/2, 288*5, 288),
                 np.arange(7, 84, 13),
        ...
        ticklabels = [['Sep. 26', 'Sep. 27', 'Sep. 28', 'Sep. 29', 'Sep. 30'],
                      ...
                      ['Spr. 15','Sum. 15','Fall 15','Win. 15','Spr. 16', 'Sum. 16']]
        fontsize=21
        for idx_loc in range(0, D, max(1,D//40)):
            fig = plt.figure(figsize=(10,7/2.5)) #(10,7) for bat
            for i in range(I):
                ax = fig.add_subplot(I,1,i+1)
                ax.plot(np.concatenate([dataa[-j][-days:].reshape(-1, D, I)[:,idx_loc, i] for j in range(1)],axis=0), label = "Actual",color='blue')
                ##ax.plot(np.concatenate([y_recv[-j][-days:].reshape(-1, D, I)[:,idx_loc, i] for j in range(days, 0 , -1)],axis=0), 'g-',
                ##        label = "Recovered", alpha = 0.8)
...
#                ax.set_title("(b) Google Dengue",**csfont, fontsize=45)
                if i==0:
                    ax.set_title(titles[data_id]+' #%d'%idx_loc, fontsize=fontsize)
            if not data_id in [1,2,3,4, 5,6]:
