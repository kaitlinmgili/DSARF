    ...
    if predict or days == 0:    
        if ID is not None:
            ...
            ylabels = [['Location x', 'Location y', 'Location z'],
                       ...
                       ['Traffic speed']]    
        fontsize= 12
        for idx_loc in range(0, D, max(1,D//5)):
...
#            days = 2
    ...
    if True:
        ...
        for i in range(200-max(dmfa.L)):
            p_s = dmfa.strans(s_t_1, z_t_1_s) # 1 * S
            p_z_mu, p_z_sig = dmfa.trans(z_t_1, None) # S*1*z_dim
            p_z_mu = dmfa.Reparam(p_z_mu, p_z_sig)
            z_val = (p_s.reshape(-1, 1, 1) * p_z_mu).sum(dim=0)
            #pdb.set_trace()
            #s = Categorical(p_s).sample()
            #z_val = p_z_mu[s].squeeze(0)
            z_values = torch.cat((z_values, z_val.unsqueeze(0)), dim = 1)
            z_t_1 = z_values.permute(1,0,2)[-np.array(dmfa.L)]
            z_t_1_s = z_values.permute(1,0,2)[-1]
            ws = np.concatenate((ws, z_val.detach().numpy()), axis = 0)
            s_t_1 = p_s * 1.0 #torch.eye(S)[s].reshape(1,-1)
...
#            ws = np.concatenate((x1,y1,x2,y2), axis = -1)
        ...
        for j in range(2):
            ...
            ys = ws[:,2*j+1]
            ax = fig.gca()
            #ax.scatter(xs.reshape(-1), ys.reshape(-1),s=5)
            ax.plot(xs.reshape(-1), ys.reshape(-1), lw=0.5)
            ax.set_xlabel("X Axis")
            ax.set_ylabel("Y Axis")
            ax.set_title("Double Pendulum")
            fig.savefig(fig_PATH+'simulation_%d.png' %j, bbox_inches='tight')
